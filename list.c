...
ip_addr_t *ip_addr_new(void) {
   ...
   ip->tcp_port_list = tcp_port_list_new();
...
void udp_port_open(ip_addr_list_t *list, in_addr_t ip, uint16_t dest_port) {
   ...
   port->status = UDP_PORT_OPEN;
}

tcp_port_t *tcp_port_new(void) {
   tcp_port_t *port;

   port = (tcp_port_t *) calloc(1, sizeof(tcp_port_t));

   return port;
}

void tcp_port_free(tcp_port_t *port) {
   free(port);
   port = NULL;
}

tcp_port_list_t *tcp_port_list_new(void) {
   tcp_port_list_t *list;

   list = (tcp_port_list_t *) calloc(1, sizeof(tcp_port_list_t));

   list->tcp_dest_port_lookup = g_hash_table_new(g_direct_hash, g_direct_equal);
   list->tcp_src_port_lookup = g_hash_table_new(g_direct_hash, g_direct_equal);

   return list;
}

tcp_port_t *tcp_port_list_lookup_dest_port(tcp_port_list_t *list,
                                           uint16_t dest_port) {
   tcp_port_t *port;

   port = (tcp_port_t *) g_hash_table_lookup(list->tcp_dest_port_lookup, 
			    GUINT_TO_POINTER((uint32_t) dest_port));
   
   return port;
}

tcp_port_t *tcp_port_list_lookup_src_port(tcp_port_list_t *list,
                                          uint16_t src_port) {
   tcp_port_t *port;

   port = (tcp_port_t *) g_hash_table_lookup(list->tcp_src_port_lookup, 
			         GUINT_TO_POINTER((uint32_t) src_port));
   
   return port;
}

void tcp_port_node_free(tcp_port_node_t *node) {
   tcp_port_free(node->port);
   node->port = NULL;
   free(node);
   node = NULL;
}

void tcp_port_list_add_port(tcp_port_list_t *list, tcp_port_t *port) {
   tcp_port_node_t *node;

   node = (tcp_port_node_t *) calloc(1, sizeof (tcp_port_node_t));
   ...
   /* Add to hash table. */
   if (port->dest_port != 0) {
      g_hash_table_insert(list->tcp_dest_port_lookup, 
                          GUINT_TO_POINTER((uint32_t)port->dest_port), 
			  (gpointer) port);
   }
...
void tcp_port_list_free(tcp_port_list_t *list) {

   tcp_port_node_t *node;

   if (list != NULL) {
      g_hash_table_destroy(list->tcp_dest_port_lookup);
      g_hash_table_destroy(list->tcp_src_port_lookup);
       
      list->tcp_dest_port_lookup = NULL;
      list->tcp_src_port_lookup = NULL;

      for (node = list->head;
           node != NULL;
           node = node->next) {
         tcp_port_node_free(node);
      }
      list->head = NULL;
      list->tail = NULL;
   }      
...
void ip_addr_list_add_ip_tcp_port(ip_addr_list_t *list, in_addr_t ip, 
   ...
   port = tcp_port_list_lookup_dest_port(addr->tcp_port_list, dest_port);
   ...
   if (port == NULL) {
      ...
      tcp_port_list_add_port(addr->tcp_port_list, port);
   }
}
